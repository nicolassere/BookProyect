...
import type { Stats, YearlyStats } from '../types';

interface YearsViewProps {
  stats: Stats;
}

// OPTIMIZATION: Memoize year card component
const YearCard = memo(({ year, index }: { year: YearlyStats; index: number }) => (
  <div 
...
// OPTIMIZATION: Memoize comparison bar component
const YearComparisonBar = memo(({ year, maxScrobbles }: { year: YearlyStats; maxScrobbles: number }) => {
...
YearComparisonBar.displayName = 'YearComparisonBar';

// OPTIMIZATION: Memoize the entire view
export const YearsView = memo<YearsViewProps>(({ stats }) => {
  const years = stats.yearlyStats || [];

  // OPTIMIZATION: Memoize max scrobbles calculation
  const maxScrobbles = useMemo(() => 
    Math.max(...years.map(y => y.totalScrobbles)),
    [years]
  );
